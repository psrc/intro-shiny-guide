...
Shiny follows a reactive programming paradigm[^1]. We don't need to command Shiny to update itself, rather, it will react on its own. If an input changes, it will automatically update outputs and other dependencies of that input. 

[^1]: https://mastering-shiny.org/basic-reactivity.html#reactive-programming

Shiny is energy-saving. It [maximizes the work not done](https://medium.com/lapis/reflection-on-principle-10-simplicity-the-art-of-maximizing-the-amount-of-work-not-done-is-99520e0654dc) and will only do the most minimal work required to update outputs.
...
Sometimes the table will render even before we finish typing a name. To delay that rapid response, we can require an event take place before proceeding with certain tasks. 

In the example below, we add an action button. The event is when the user clicks on 'Enter'.
