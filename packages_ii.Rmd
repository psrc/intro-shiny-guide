...
In the previous example, to remove the last three columns from the table we manipulated the data frame itself. In lieu of altering the data, it's possible to use DT to simply hide the columns using the `columnDefs` argument within `options`. There are, however, several quirks to know!

1) The targets argument require a numeric vector. Column names don't work.
2) The first column in the DT is 0.
...
Additionally, [styles](https://rstudio.github.io/DT/010-style.html) can also be applied to table cells with the help of a few helping functions:
...
# Server
 ...
 output$main_table <- renderDT({
    ...
    datatable(df,
      ...
      # conditional styling of cell value. The font color of 'name' is based on 'sex'
      formatStyle('name',
                  'sex',
